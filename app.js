...
function parseLicenseFromMemo(memo) {
  // Extract software items from SL memo sections
  const licenses = [];
  if(memo.type !== 'sl' || memo.status !== 'completed') return licenses;

  // Get purchase date from memo
  const purchaseDate = memo.approvedAt || memo.updatedAt || memo.createdAt;

  // Parse from sections[0] which is the software table
  ...
  rows.forEach(row => {
    ...
    licenses.push({ name, seats, pricePerMonth: price, months, purchaseDate, expiry: expiry.toISOString(), project: memo.project, memoNo: memo.memoNo });
  });
...
function renderLicense() {
  // Collect all licenses from completed SL memos
  ...
  const urgentLics = licenses.filter(lic => {
    const s = getLicenseStatus(lic.expiry);
    return s.days >= 0 && s.days <= 30;
  }).sort((a,b) => new Date(a.expiry) - new Date(b.expiry));

  if(urgentLics.length) {
    alertSection.style.display = 'block';
    alertList.innerHTML = urgentLics.map(lic => {
      const s = getLicenseStatus(lic.expiry);
  ...
  // Sort: expired first, then expiring, then active — all by expiry date
