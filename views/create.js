// ─────────────────────────────────────────────
//  views/create.js  —  form, type selection, PDF collect
// ─────────────────────────────────────────────
...
// ── Row helpers ──
const TRASH_SVG = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>`;
...
function addHWRow() {
  ...
  d.innerHTML = `<input class="ri" type="text" placeholder="ชื่ออุปกรณ์"><input class="ri hw-price" type="number" placeholder="ราคา" oninput="calcHW()"><input class="ri hw-qty" type="number" placeholder="จำนวน" oninput="calcHW()"><button class="rm-btn" onclick="rmRow(this,'hw-rows');calcHW()" title="ลบ">${TRASH_SVG}</button>`;
...
function addName(cid, cls, doCalc) {
  ...
  d.innerHTML = `<span class="name-num">${n}.</span><input class="ri ${cls}" type="text" placeholder="ชื่อ-นามสกุล ตำแหน่ง" oninput="${doCalc?'calcINT()':''}"><button class="rm-btn" onclick="rmName(this,'${cid}')${calc}" title="ลบ">${TRASH_SVG}</button>`;
  c.appendChild(d);
}
...
function _buildAcctRow(email, vals, n) {
  ...
  h += `<td style="padding:3px 4px;border:1px solid var(--border);text-align:center"><button class="rm-btn" onclick="this.closest('tr').remove()" style="width:24px;height:24px" title="ลบ">${TRASH_SVG}</button></td>`;
  return h;
}
function addAcctRow() {
...
function memoSubject(data) {
  ...
  return ({ sl:`ขออนุมัติจัดซื้อ Software License สำหรับ${p}`, hw:`ขออนุมัติจัดซื้อ Hardware สำหรับ${p}`,
            int:`ขออนุมัติค่าใช้จ่าย Team Activity สำหรับ${p}`, ent:`ขออนุมัติค่าใช้จ่ายรับรองลูกค้าสำหรับ${p}`,
...
function collectMemoData() {
  ...
  const data = {
    ...
    memoNo: val('#f-memo-no'), date: val('#f-date'),
    project: val('#f-project'), to: val('#f-to'),
    subject: val('#f-subject'), reason: selectedReason(),
    reviewerName: revInputs[0]||'-', reviewerTitle: revInputs[1]||'-', reviewerDate: revInputs[2]||TODAY,
    approverName: revInputs[3]||'-', approverTitle: revInputs[4]||'-', approverDate: revInputs[5]||TODAY,
...
function validateMemo(data) {
  ...
  if(!data.project) missing.push('โครงการ');
